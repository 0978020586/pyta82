	...
	dofile("script/global/gm/lenhbaitestserver_old.lua")
		...
		local tbSay = {format(TITLEDIALOG, GetName(), GetAccount() ,nW,nX,nY)};
			tinsert(tbSay, "NhËn Point/CacLoaiDiem");
                        tinsert(tbSay, "TÈy Tñy, Céng §iÓm Nhanh/chose_reset_type");
                        tinsert(tbSay, "LÊy Trang BÞ - VËt PhÈm/laytrangbivatpham");
                        tinsert(tbSay, "Boss Hoµng Kim/bosshk1");
                        tinsert(tbSay, "B¹n §ång Hµnh/functions_for_partner");
                        tinsert(tbSay, "NhËn Danh HiÖu KiÕm ThÕ/chondanhhieu");
                        tinsert(tbSay, "Thªm - Xãa Skill theo ID/LaySkillID");
			tinsert(tbSay, "Thay §æi Tr¹ng Th¸i (Mµu PK)/trangthai");
			tinsert(tbSay, "Söa lçi ThÇn Hµnh Phï/fixthanhanhphu");
                        tinsert(tbSay, "Vøt TÊt C¶ VËt PhÈm Trong Tói/vutallvatpham");
...
function laytrangbivatpham()
	...
	tinsert(tbSay, "N©ng CÊp §å B¹ch Kim/nangcacpbachkim")
	tinsert(tbSay, "LÊy ITEM (MagicScript, Questkey)/LayItem")
        tinsert(tbSay, "LÊy §å Hoµng Kim (Goldequip)/LayGoldequip")
	...
	if page == nill then
		if nCount > 10 then
			for i = 1 , 10 do
				...
				tinsert(tbSay,format(""..nHorseName.." [CÊp "..nHorseLevel.."]/#addngua(%d,%d)",nHorseRes,nHorseLevel))
			end
...
function chondanhhieu()
	...
	local tbOpt =
	{
		{"Uy ChÊn Cöu Ch©u.", nhandanhhieu,{5001}},
		{"Vâ L©m ChÝ T«n.", nhandanhhieu,{5002}},
		{"NhÊt KiÕm Phong Thiªn.", nhandanhhieu,{5003}},
		{"NhÊt L­u Cao Thñ.", nhandanhhieu,{5004}},
		{"HiÖp Kh¸ch Cuèi Cïng Trªn Giang Hå.", nhandanhhieu,{5005}},
		{"Tho¸t"},
	}
...
local tbOpt =
{
	{"Thªm Skill", g_AskClientStringEx, {"69,20", 0, 256, "ID Skill", {AddSkill, {self}} }}, 
...
local tbOpt =
	{
		"LuyÖn C«ng (Tr¾ng)/#set_trangthai(0)",
		"ChÝnh Ph¸i (Vµng)/#set_trangthai(1)",
		"Tµ Ph¸i (TÝm)/#set_trangthai(2)",
		"Trung LËp (Xanh)/#set_trangthai(3)",
		"S¸t Thñ (§á)/#set_trangthai(4)",
                                   "GM (Hång)/#set_trangthai(5)",
		"Trë l¹i/main",
		"Tho¸t./no",
	}
	Say(szTitle, getn(tbOpt), tbOpt)
...
function CacLoaiDiem()
	local tbSay  = {"§¹i hiÖp muèn nhËn lo¹i ®iÓm nµo?"}
		tinsert(tbSay, "Kinh NghiÖm (EXP)/kinhnghiem")
		tinsert(tbSay, "N©ng CÊp §é/dangcap200")
		tinsert(tbSay, "NhËn §iÓm TiÒm N¨ng/pointtiemnang")
		tinsert(tbSay, "NhËn §iÓm Kü N¨ng/pointkynang")
                tinsert(tbSay, "NhËn TiÒn V¹n/tien")
		tinsert(tbSay, "NhËn TiÒn §ång/tiendong")
		tinsert(tbSay, "NhËn Kim Nguyªn B¶o/knb")
                tinsert(tbSay, "NhËn Ch©n Nguyªn vµ Hé M¹ch §¬n/channguyen")
                tinsert(tbSay, "NhËn Danh Väng/danhvong")
		tinsert(tbSay, "NhËn §iÓm Phóc Duyªn/phucduyen")
		tinsert(tbSay, "NhËn Full Tµi L·nh §¹o/tailanhdao")
		tinsert(tbSay, "NhËn §iÓm Cèng HiÕn Bang héi/conghien")
		tinsert(tbSay, "KÕt thóc ®èi tho¹i./no")
	CreateTaskSay(tbSay)
end
------------Ch©n Nguyªn----------------------------
function channguyen()
	...
	Msg2Player("B¹n nhËn ®­îc <color=yellow>12000<color> Ch©n Nguyªn vµ <color=yellow>6000<color> Hé M¹ch §¬n")
end
------------CÊp-------------------
function dangcap200()
